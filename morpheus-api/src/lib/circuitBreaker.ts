type EstadoCircuito = {
  estado: 'fechado' | 'aberto' | 'meio-aberto'
  falhas: number
  sucessos: number
...
const CONFIG = {
  limiteFalhas: 3,
  timeoutRecuperacao: 60_000,
  limiteSuccessosRecuperacao: 2,
}

function obterEstado(nomeTool: string): EstadoCircuito {
...
export function podeExecutar(nomeTool: string) {
...
export function registrarSucesso(nomeTool: string) {
  const circuito = obterEstado(nomeTool)
...
export function registrarFalha(nomeTool: string) {
  const circuito = obterEstado(nomeTool)
  circuito.falhas += 1
  circuito.sucessos = 0
