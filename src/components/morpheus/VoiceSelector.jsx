  ...
  return (
    <div className="voice-selector">
      <style>{`
        ...
        .voice-preview-btn--playing { border-color: var(--cyan); background: rgba(0,255,255,0.15); animation: pulse-cyan 1s ease-in-out infinite; }
        .voice-preview-btn--done { border-color: #00ff66; color: #00ff66; }
        .voice-check { color: var(--cyan); width: 16px; height: 16px; }
      `}</style>
        ...
        <div>
          ...
          <div className="voice-grid">
            {femaleVoices.map(voice => (
              <div
                key={voice.id}
                className={`voice-card ${voice.id === currentVoice ? 'voice-card--active' : ''}`}
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  {voice.id === currentVoice && <Check className="voice-check" />}
                  <button
                    ...
                    title="Preview"
                  >
                    {previewingId === voice.id ? <Square size={10} /> : <Play size={10} />}
                  </button>
                </div>
              </div>
            ))}
            ...
            {maleVoices.map(voice => (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  {voice.id === currentVoice && <Check className="voice-check" />}
                  <button
                    className={`voice-preview-btn ${previewingId === voice.id ? 'voice-preview-btn--playing' : ''}`}
                    onClick={(e) => { e.stopPropagation(); handlePreview(voice) }}
                    disabled={previewingId === voice.id}
                    title="Preview"
                  >
      ...
      {unavailable.length > 0 && (
        <div>
          <div className="voice-group-title">Indisponiveis ({unavailable.length})</div>
          <div className="voice-grid">
            {unavailable.map(voice => (
              <div key={voice.id} className="voice-card voice-card--unavailable">
                <div className="voice-info">
                  <span className="voice-name">{voice.name}</span>
                  <span className="voice-accent">offline</span>
                </div>
              </div>
            ))}
          </div>
